  ...
  function check_words() {
    for (const [category, answers] of Object.entries(solution)) {
      ...
      for (const word in $selected) {
        if (answers.includes($selected[word].name)) {
          found_words += 1;
        }
      }
...
<main>
  <p class="mb-8">Create groups of four!</p>
  ...
  <button class="bg-gray-500 rounded-full mt-8 p-2" on:click={generate_words}>
    Regenerate Puzzle
  </button>
  <button class="bg-gray-500 rounded-full p-2" on:click={shuffle_words}>
    Shuffle
  </button>
  <button class="bg-gray-500 rounded-full p-2" on:click={() => grid.onReset()}>
    Deselect All 
  </button>
  <button class="bg-gray-500 rounded-full p-2 disabled:opacity-50" on:click={check_words} disabled={$selected.length < 4}>
    Submit
  </button>
